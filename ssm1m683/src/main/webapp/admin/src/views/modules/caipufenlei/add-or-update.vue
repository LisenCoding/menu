<template>
  <div class="addEdit-block">
    ...
    >
      <el-row >
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="菜谱分类" prop="caipufenlei">
          <el-input v-model="ruleForm.caipufenlei" 
              placeholder="菜谱分类" clearable  :readonly="ro.caipufenlei"></el-input>
        </el-form-item>
        <div v-else>
...
export default {
  data() {
	...
	caipufenlei : false,
      ...
      rules: {
          caipufenlei: [
                { required: true, message: '菜谱分类不能为空', trigger: 'blur' },
          ],
      }
    };
  },
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `caipufenlei/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.caipufenleiCrossAddOrUpdateFlag = false;
						 this.parent.search();
